...
task :setup do
  ...
  sh "cd #{test};echo user | RAILS_ENV=test rails g goc:setup --ratings --domains;"
end
...
task :adding_badges do
  sh "cd #{test};RAILS_ENV=test rake goc:add_badge[noob,100,comments]"
  sh "cd #{test};RAILS_ENV=test rake goc:add_badge[medium,500,comments]"
  sh "cd #{test};RAILS_ENV=test rake goc:add_badge[hard,1000,comments]"
end
